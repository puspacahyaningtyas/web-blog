...
class M_institutions_lifter extends CI_Model {
	...
	public function get_where($keyword, $limit = 0, $offset = 0, $sort_field = '', $sort_type = 'ASC') {
		$this->db->select('id, group, option, is_deleted');
		$this->db->where('group', 'institutions_lifter');
		$this->db->group_start();
	...
	public function get_all() {
		return $this->db
			->select('id, option')
			->where('group', 'institutions_lifter')
			->get(self::$table);
	}
