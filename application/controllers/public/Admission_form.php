...
class Admission_form extends Public_Controller {
	...
	public function index() {
		...
		$this->vars['page_title'] = 'Formulir Penerimaan '. ($this->session->userdata('school_level') == 5 ? 'Mahasiswa' : 'Peserta Didik').' Baru Tahun '.$this->session->userdata('admission_year');
	...
	private function fill_data() {
		return [
			'first_choice' => $this->input->post('first_choice') ? $this->input->post('first_choice', true) : null,
			'second_choice' => $this->input->post('second_choice') ? $this->input->post('second_choice', true) : null,
			'prev_exam_number' => $this->input->post('prev_exam_number') ? $this->input->post('prev_exam_number', true) : null,
			'paud' => $this->input->post('paud') ? $this->input->post('paud', true) : null,
			'tk' => $this->input->post('tk') ? $this->input->post('tk', true) : null,
			...
			'diploma_number' => $this->input->post('diploma_number') ? $this->input->post('diploma_number', true) : null,
			...
			'sibling_number' => $this->input->post('sibling_number'),
			'is_prospective_student' => 'true',
			're_registration' => 'false',
			'registration_number' => $this->m_registrants->registration_number()
		];
	}
	...
	private function validation() {
		...
		if ($this->session->userdata('school_level') == 4 || $this->session->userdata('school_level') == 5) {
		...
		if ($this->session->userdata('school_level') == 2 || $this->session->userdata('school_level') == 3 || $this->session->userdata('school_level') == 4) {
			$val->set_rules('skhun', 'Nomor Seri SKHUN Sebelumnya', 'trim|numeric|min_length[50]|max_length[50]');
			$val->set_rules('diploma_number', 'Nomor Seri Ijazah Sebelumnya', 'trim|numeric|min_length[50]|max_length[50]');
			$val->set_rules('nisn', 'NISN', 'trim');
		}
		if ($this->session->userdata('school_level') != 1) {
		...
		$val->set_rules('kps_receiver', 'Penerima KPS / PKH / KIP', 'trim|required|in_list[true,false]');
		$val->set_rules('kps_number', 'No. KPS/KKS / PKH / KIP', 'trim');
